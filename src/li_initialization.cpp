...
bool data_accum_finished = false, data_accum_start = false,
...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr &msg) {
  ...
  // !timediff_set_flg && !imu_deque.empty()) {
  ...
  if ((lidar_type == VELO16 || lidar_type == OUST64 ||
       lidar_type == HESAIxt32) &&
      cut_frame_init) {
...
void imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in) {
  ...
  msg->header.stamp =
      get_ros_time(get_time_sec(msg_in->header.stamp) - timediff_imu_wrt_lidar -
      ...
      if (p_imu->imu_need_init_) {
        ...
        while (imu_time < lidar_end_time) {
          ...
          if (imu_deque.empty())
            break;
          imu_time =
              get_time_sec(imu_deque.front()->header.stamp); // can be changed
          imu_next = *(imu_deque.front());
        }
      }
      imu_pushed = true;
