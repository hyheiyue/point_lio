...
std::vector<double> first_frame_gravity;
bool recive_frist_frame = false;
...
PointCloudXYZI::Ptr loadPointcloudFromPcd(const std::string &file_path) {
  ...
  RCLCPP_INFO(LOGGER, "Loaded %zu points from %s", pcd_ptr->size(),
              file_path.c_str());
  return pcd_ptr;
}
...
template <typename T> void set_twist(T &out) {
  if (use_imu_as_input) {
...
int main(int argc, char **argv) {
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (imu_en && align_gravity_en && p_imu->imu_need_init_) {
        ...
        if (!p_imu->imu_need_init_) {
          Eigen::Vector3d first_frame_gravity = -p_imu->mean_acc.normalized();
          if (first_frame_gravity.allFinite()) {
            gravity = {first_frame_gravity[0], first_frame_gravity[1],
                       first_frame_gravity[2]};
            gravity_init = gravity;
            p_imu->gravity_ << VEC_FROM_ARRAY(gravity);
            recive_frist_frame = true;

            RCLCPP_INFO(LOGGER, "gravity initialized: %f, %f, %f", gravity[0],
                        gravity[1], gravity[2]);

            Eigen::Matrix3d rot_init;
            p_imu->Set_init(first_frame_gravity, rot_init);
            kf_input.x_.rot = rot_init;
            kf_output.x_.rot = rot_init;
            kf_output.x_.acc =
                -rot_init.transpose() *
                Eigen::Vector3d(gravity[0], gravity[1], gravity[2]);

          } else {
            RCLCPP_WARN(LOGGER,
                        "Invalid gravity direction, skipping initialization.");
            continue;
          }
        } else {
          continue;
